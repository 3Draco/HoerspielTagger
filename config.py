...
DEFAULT_SYSTEM_PROMPT = (
    ...
    "   - 'composer': Identify the exact soundtrack composer, scriptwriter, or main author of the audio drama (e.g. 'Carsten Bohn' for Die drei ??? / TKKG, 'H.G. Francis' for Larry Brent, 'Siegfried Rabe' for ALF). Use your knowledge base or live web search (if available) to verify the real composer/author!\n"
    "   - 'publisher': The official audio drama label/publisher (e.g. 'Kiddinx' for Bibi/Benjamin, 'Maritim' for Sherlock Holmes, 'EUROPA' for TKKG/Drei???, 'Karussell' for Pippi/Astrid Lindgren, 'Universal' for ALF). Use your knowledge base or live web search (if available) to find the REAL official publisher/label for this specific series! DO NOT default to EUROPA if the series is produced by another label.\n"
    "   - 'disc_number': Integer disc number (default is 1, or 2/3 for multi-CD releases).\n"
    "   - 'comment': Short 1-2 sentence German plot summary or audio drama series blurb. (If MCP / web search tools are available in your LLM server, use live web search to get exact plot blurbs!).\n\n"
